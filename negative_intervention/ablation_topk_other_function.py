...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask_ratio', type=int, default=75)
    ...
    sampled_functions = functions_names.remove(args.function_name)
    arg_function_head_features = importances[args.function_name][:args.mask_ratio]
    for sampled_function in sampled_functions:
        ...
        if args.use_head == "randomk":
            ...
            head_features = importances[sampled_function][:args.mask_ratio]
            ...
            if sampled_function != args.function_name:
                head_features = []
                for each_head in importances[sampled_function]:
                    if each_head not in arg_function_head_features and len(head_features) <= args.mask_ratio:
                        head_features.append(each_head)

            else:
                head_features = importances[sampled_function][:args.mask_ratio]
        elif args.use_head == "elbow":
            head_features = [i[0] for i in sorted_indices[sampled_function][0:elbow[args.function_name]]]
        ...
        data = [args.function_name, sampled_function, args.model_name, args.use_head, args.mask_ratio, head_score, acc_num / len(selected_prompts), args.token_use]
