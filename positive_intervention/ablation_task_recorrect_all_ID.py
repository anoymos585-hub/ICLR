...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder_dir', type=str, default='pos_intervention_id')
    ...
    for each in check_dataset2:
        check_dataset.append("../"+each["image_path"])
    prompts, labels, answers, images, original_questions, subquestions = cot_prompt(check_dataset1)
    ...
    for k in tqdm(range(len(selected_prompts)), desc="Processing Prompts"):
        ...
        if base_scores["llm"] == 1.0 and scores["llm"] == 0.0:
            with open(os.path.join(f"./{args.folder_dir}/{args.model_name}", args.function_name.replace(" ", "_"), args.function_name.replace(" ", "_") + "_" + args.use_head +str(args.use_layer_bias)+ "_adv_incorrect.jsonl"), "a") as f:
    ...
    data = [args.function_name, args.model_name, args.use_head, args.mask_ratio, head_score, acc_num / len(prompts), args.token_use, args.alpha, args.sign]
    base_data= [args.function_name, args.model_name, args.use_head, args.mask_ratio, base_head_score, acc_num / len(prompts), args.token_use, args.alpha, args.sign]
