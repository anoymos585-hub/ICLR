...
# def get_llama_activations_pyvene(tokenizer, collected_model, collectors, prompt, device):
#     with torch.no_grad():
#         prompt = prompt.to(device)
#         output = collected_model({"input_ids": prompt, "output_hidden_states": True})[1]
#         # logits = output.logits  # shape: [batch_size, seq_len, vocab_size]
#         # predicted_token_ids = torch.argmax(logits, dim=-1)  # 获取预测 token id
#         # decoded = tokenizer.batch_decode(predicted_token_ids, skip_special_tokens=True)        
#         generated_ids = collected_model.generate(
#             input_ids=prompt,
#             max_new_tokens=32,
#             do_sample=False,  # 如果你希望 deterministic（贪婪解码）
#             output_hidden_states=True,
#             return_dict_in_generate=True  # 关键参数，允许我们拿到隐藏状态
#         )
    
#         # 解码生成的 token（含原始输入和新生成的）
#         decoded_text = tokenizer.batch_decode(generated_ids.sequences, skip_special_tokens=True)
#         print("Generated output:", decoded_text)
#     hidden_states = output.hidden_states
#     hidden_states = torch.stack(hidden_states, dim = 0).squeeze()
#     hidden_states = hidden_states.detach().cpu().numpy()
...
# 在 generated_ids 中搜索 output_text_ids 的起始位置
...
#     # 只匹配 "answer": 后面的内容
#     json_text = re.sub(r'("answer"\s*:\s*)(.+?)([,}])', escape_inner_quotes, json_text)

#     # 再修复 answer 后的值如果没有外部双引号，加上
#     def fix_answer_value(match):
#         key, value, ending = match.group(1), match.group(2).strip(), match.group(3)
#         if value.startswith('"') and value.endswith('"'):
#             return f'{key}{value}{ending}'
#         else:
#             return f'{key}"{value}"{ending}'

#     fixed_text = re.sub(r'("answer"\s*:\s*)([^,}]+)([,}])', fix_answer_value, json_text)

#     try:
#         return json.loads(fixed_text)
#     except Exception as e:
#         print(f"[safe_json_parse] JSON parsing failed! Error: {e}")
#         print(f"[safe_json_parse] Fixed text:\n{fixed_text}")
#         return None
...
def get_qwen_activations_pyvene(tokenizer, collected_model, collectors, prompt, device, args, image_path):
    ...
    with torch.no_grad():
        ...
        #print(model_inputs)
        #qprint([tokenizer.tokenizer.decode([tid]) for tid in model_inputs["input_ids"][0]])
        # generated_ids = collected_model.generate(
        #     **model_inputs,
        ...
        # matches = re.findall(r'"answer":\s*"([^"]+)"', output_text)

        # if matches:
        #     print("Extracted answers:")
        #     print(matches[-1]["answers"])
        #     output_answer = matches[-1]
        # else:
    ...
    # selected_indices = random.sample(token_positions, min(5, len(token_positions)))
    # print("Selected token positions:", selected_indices)
...
def get_intern_activations_pyvene(tokenizer, collected_model, collectors, prompt, device, args, image_path):
    ...
    with torch.no_grad():
        ...
        model_inputs = tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=True, return_dict=True, return_tensors="pt").to(device, dtype="auto")
        generated_ids, adv_generated = collected_model.generate(
            base=model_inputs,
            max_new_tokens=128,
            do_sample=False,
...
def get_gemma_activations_pyvene(tokenizer, collected_model, collectors, prompt, device, args, image_path):
    ...
    with torch.no_grad():
        ...
        model_inputs = tokenizer(text="<image>"+prompt, images=image, return_tensors="pt").to(device, dtype="auto")
...
- You can use the information in prior_knowledge to help you answer the question.
...
Your answer:
"""
# - You can use the information of prior_knowledge to help you answer the subquestion.
# - You can use the information in the main question and prior_knowledge to help you answer the subquestion.
...
# - Answer the subquestion carefully.
# - Use relevant information from the prior chain-of-thought to inform your answer.
# - Keep your answer focused, logically sound, and concise.
# - Only output a single JSON list with your answer.
# - Do NOT include any extra text, code, or commentary.
# - Do NOT repeat the question or prior knowledge.
# - Only respond with the final answer in the format below.
...
def cot_prompt(dataset, low_function_name=None, high_function_name=None):
    ...
    return all_prompts, all_labels, all_answers, all_images, all_questions, all_subquestions

def cot_prompt_OOD(dataset, check_dataset, num=100, low_function_name=None, high_function_name=None):
    all_prompts = []
    all_labels = []
    all_answers = []
    all_images = []
...
def cot_prompt_only_correct(dataset, check_dataset, low_function_name=None, high_function_name=None):
    all_prompts = []
    all_labels = []
    all_answers = []
    all_images = []
...
def adv_generate(collected_model, tokenizer, prompt, device, image, model_name):
    ...
    with torch.no_grad():
        if "Qwen" in model_name:
            ...
            model_inputs = tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=True, return_dict=True, return_tensors="pt").to(device)
            base_generated, adv_generated = collected_model.generate(
                base=model_inputs,
                max_new_tokens=128,
                do_sample=False,
    ...
    return base_generated, adv_generated, model_inputs
...
def get_answer_intervention(base_generated, tokenizer): 
    ...
    print(output_text)
    if "Your answer:\n\nassistant\n" in output_text:
        output_text = output_text.split("Your answer:\n\nassistant\n")[1]
        #json_blocks = re.findall(r'\[\s*\{.*?\}\s*\]', output_text, flags=re.DOTALL)
    else:
        output_text = output_text.split("Your answer:")[1]
    #print(output_text)
    json_blocks = re.findall(r'\[\s*\{.*?\}\s*\]', output_text, flags=re.DOTALL)
    results = []
    explanation = ""
    ...
    for block in json_blocks:
        try:
            ...
            if isinstance(data, list) and isinstance(data[0], dict):
                ...
                final_answer = str(data[0].get("answer", "")).strip()
                #print(explanation)
                #print(final_answer)
                results.append({
    ...
    if not results:
        explanations = re.findall(r'"explanation"\s*:\s*"((?:[^"\\]|\\.)*)"', output_text, flags=re.DOTALL)
        final_answers = re.findall(r'"answer"\s*:\s*"((?:[^"\\]|\\.)*)"', output_text, flags=re.DOTALL)
        if explanations:
...
def evaluate_metrics(predictions, references, predictions_full, references_full, sources=None, args=None, evaluate_model=None):
    ...
    results["bleu"] = bleu_score["precisions"][0]
...
def simi_scoring(prompt, label, source_answer, image, model_name, tokenizer, intervened_model, device, args, evaluate_model):
    ...
    # with torch.no_grad():
        ...
        #     return_dict_in_generate=True
        # )
    # generated_tokens = base_generated.sequences
    # logits = base_generated.scores
    # base_answer = get_answer(generated_tokens, tokenizer)
    base_generated, adv_generated, model_inputs = adv_generate(intervened_model, tokenizer, prompt, device, image, model_name)
    ...
    #print(adv_answer)
    if args.use_bleu:
        base_generated_only_ids = base_tokens[0][model_inputs["input_ids"].shape[1]:].tolist()
        # adv_generated_ids = adv_tokens[0]
        adv_generated_only_ids = adv_tokens[0][model_inputs["input_ids"].shape[1]:].tolist()
    ...
    # print(base_answer_full)
    # print(adv_answer_full)
    # base_text_ids = tokenizer.encode(base_answer, add_special_tokens=False)
    # adv_text_ids = tokenizer.encode(adv_answer, add_special_tokens=False)
    # base_start_ids = find_subsequence(base_text_ids, base_generated_only_ids)
    # adv_start_ids = find_subsequence(adv_text_ids, adv_generated_only_ids)
    
    # print("Base answer:", base_answer, "Adv answer:", adv_answer)
    ...
    # kl_scores = kl_evaluate(base_logits, adv_logits, base_start_ids, adv_start_ids)
    # if label in ["Knowledge Recall", "Semantic Understanding", "Syntactic Understanding"]:
    #     score = semantic_similarity (base_answer, adv_answer)
    # scores = {
    #     "bleu": 0,
    #     "cosine": 0,
    #     "comet": 0,
    ...
    # }
    # else:
    #     score = value_similarity(base_generated, adv_generated)
    # score_bleu = scores["bleu"]
    # score_rouge = scores["rouge1"]
    # score_comet = scores["comet"]
...
def simi_scoring_intervention(prompt, label, source_answer, image, model_name, tokenizer, intervened_model, device, args, evaluate_model):
    ...
    base_answer, base_explanation = get_answer_intervention(base_tokens, tokenizer)
    adv_answer, adv_explanation = get_answer_intervention(adv_tokens, tokenizer)
    # base_answer, adv_answer = None, None
    ...
    #base_scores = evaluate_metrics_intervention([str(base_answer)], [str(adv_answer)], [str(adv_answer_full)], [str(base_answer_full)], sources=[str(source_answer)], args=args, evaluate_model=evaluate_model)  
    scores = {
        "bleu": 0,
        "cosine": 0,
        "comet": 0,
    ...
    return base_answer, base_explanation, adv_answer, adv_explanation, scores, base_scores, kl_scores
...
def get_intervation_result(prompt, tokenizer, intervened_model, device, args):
    ...
    #     index = len(base_generated_only_ids) - base_generated_only_ids[::-1].index(151668)
    # except ValueError:
    #     index = 0
    # base_thinking_content = tokenizer.decode(base_generated_only_ids[:index], skip_special_tokens=True).strip("\n")
    # base_content = tokenizer.decode(base_generated_only_ids[index:], skip_special_tokens=True).strip("\n")
    
    # try:
    #     # rindex finding 151668 (</think>)
    #     index = len(adv_generated_only_ids) - adv_generated_only_ids[::-1].index(151668)
    # except ValueError:
    #     index = 0
    # adv_thinking_content = tokenizer.decode(adv_generated_only_ids[:index], skip_special_tokens=True).strip("\n")
    # adv_content = tokenizer.decode(adv_generated_only_ids[index:], skip_special_tokens=True).strip("\n")
