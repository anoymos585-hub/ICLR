...
from utils import tokenized_tqa, cot_prompt, get_qwen_activations_pyvene, get_intern_activations_pyvene, get_gemma_activations_pyvene
...
def main(): 
    ...
    dataset = json.load(open(f'./dataset/{args.mode}_1000_final.json'))
    dataset_name = f'{args.mode}_1000'
    prompts, labels, answers, images = cot_prompt(dataset)
    # print(prompts)
    # print(labels)
    # print(answers)
    collectors = []
    ...
    with open("./head_results/output_" + args.model_name + "_" + dataset_name + "_" + args.mode + ".json", "w") as f:
    ...
    with open(f'./head_results/{args.model_name}_{dataset_name}_token_positions_{args.mode}.pkl', 'wb') as f:
    ...
    if ("7B" in args.model_name or "8B" in args.model_name or "4b" in args.model_name) and args.mode=="train":
        with open(f'./head_results/{args.model_name}_{dataset_name}_head_wise_{args.mode}_0.pkl', 'wb') as f:
            pickle.dump(all_head_wise_activations[0:quarter], f)
        with open(f'./head_results/{args.model_name}_{dataset_name}_head_wise_{args.mode}_1.pkl', 'wb') as f:
            pickle.dump(all_head_wise_activations[quarter:2*quarter], f)
        with open(f'./head_results/{args.model_name}_{dataset_name}_head_wise_{args.mode}_2.pkl', 'wb') as f:
            pickle.dump(all_head_wise_activations[2*quarter:3*quarter], f)
        with open(f'./head_results/{args.model_name}_{dataset_name}_head_wise_{args.mode}_3.pkl', 'wb') as f:
            pickle.dump(all_head_wise_activations[3*quarter:], f)
    else:
        with open(f'./head_results/{args.model_name}_{dataset_name}_head_wise_{args.mode}.pkl', 'wb') as f:
